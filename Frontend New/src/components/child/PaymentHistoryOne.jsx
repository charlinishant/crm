...
const PaymentHistoryOne = ({ trashMode = false }) => {
  ...
  const runBulkLeadAction = async () => {
    const ids = tableSelection.selectedIds.map(Number).filter(Number.isInteger);
    ...
    const token = localStorage.getItem("authToken");
    const actionConfig = {
      trash:{ url:`${API_URL}/leads/bulk/permanent`, method:"DELETE", success:"Selected leads permanently deleted from database." },
      ...
      <div  className="table-section fa-2x">
        ...
        {bulkAction && (
          <BulkDeleteConfirmation
            ...
            title={
              bulkAction === "restore"
                ? `Restore ${tableSelection.selectedCount} selected leads?`
                : bulkAction === "permanent"
                  ? `Permanently delete ${tableSelection.selectedCount} selected leads?`
                  : `Delete ${tableSelection.selectedCount} selected leads?`
            }
            message={
              bulkAction === "restore"
                ...
                : bulkAction === "permanent"
                  ...
                  : "This action cannot be undone. The selected leads will be deleted from the database."
            }
