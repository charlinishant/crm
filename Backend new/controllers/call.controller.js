...
const normalizeRecordingUrl = (value) => {
  ...
  try {
    ...
    if (
      ...
      String(process.env.ALLOW_INSECURE_RECORDING_URLS || "false").toLowerCase() !== "true"
    ) return ""
...
const getMcubeWebhookData = async (req, fallbackDirection = "outbound") => {
  ...
  const rawDirection = String(getMcubePayloadValue(req, ["direction", "calltype", "type"]) || fallbackDirection).toLowerCase()
  const direction = (
    rawDirection === "outbound" ||
    rawDirection === "inbound"
  )
    ? rawDirection
    : isConfiguredInboundNumber(virtualNumber) ? "inbound" : fallbackDirection
...
exports.mcubeRecordingWebhook = async (req, res) => {
  try {
    const explicitDirection = String(getMcubePayloadValue(req, ["direction", "calltype", "type"]) || "").trim().toLowerCase()
    const fallbackDirection = explicitDirection === "outbound" ? "outbound" : "inbound"
    const result = await saveMcubeWebhookCall(req, fallbackDirection, { requireRecording:true })
    res.status(200).json(result)
  } catch (error) {
