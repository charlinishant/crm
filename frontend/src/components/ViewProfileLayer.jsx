...
const ViewProfileLayer = () => {
    ...
    const readURL = (event) => {
        ...
        reader.onload = async (readerEvent) => {
            ...
            if (storedUser.id) {
                try {
                    const response = await fetch(`${process.env.REACT_APP_API_URL || "http://localhost:5000"}/users/${storedUser.id}`, {
                        method: "PATCH",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({ profilePhoto: nextImage }),
                    });
                    if (!response.ok) throw new Error("Unable to save profile photo");
                    const updatedUser = { ...storedUser, profilePhoto: nextImage };
                    localStorage.setItem("authUser", JSON.stringify(updatedUser));
                } catch (error) {
                    setImagePreview(storedUser.profilePhoto || "");
